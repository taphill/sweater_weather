...
RSpec.describe 'Api/V1/Road trip request', type: :request do
  describe 'POST /api/v1/road_trip', :vcr do
    ...
    context 'when origin is not a location' do
      let(:user) { create(:user) }
    ...
    context 'when origin is a number' do
      let(:user) { create(:user) }

      before do
        post api_v1_road_trip_index_path,
             params: { origin: '0009234', destination: 'nashville,tn', api_key: user.api_key }
      end
    ...
    context 'when destination is not a location' do
      let(:user) { create(:user) }

      before do
        post api_v1_road_trip_index_path,
             params: { origin: 'denver,co', destination: 'skjhfasiwkjahd', api_key: user.api_key }
      end
    ...
    context 'when destination is a number' do
      let(:user) { create(:user) }

      before do
        post api_v1_road_trip_index_path,
             params: { origin: 'denver,co', destination: '0394983', api_key: user.api_key }
      end
    ...
    context 'when route is impossible' do
      ...
      it { expect(json_body[:data][:attributes][:end_city]).to be_a(String) }
      it { expect(json_body[:data][:attributes]).to have_key(:travel_time) }
      it { expect(json_body[:data][:attributes][:travel_time]).to eq('impossible route') }
      it { expect(json_body[:data][:attributes]).to have_key(:weather_at_eta) }
      it { expect(json_body[:data][:attributes][:weather_at_eta]).to be_nil }
    end

    context 'when route is too long for weather eta' do
      let(:user) { create(:user) }

      before do
        post api_v1_road_trip_index_path,
             params: { origin: 'new york city,ny', destination: 'fairbanks,ak', api_key: user.api_key }
      end
